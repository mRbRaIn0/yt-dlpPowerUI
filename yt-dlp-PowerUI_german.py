...
import time
import glob
import random  # Wichtig für die Pause
import json
import io
...
class YtDlpUI(ctk.CTk):
    def __init__(self):
        ...
        # Fenstergröße und zentrierte Position berechnen
        win_width = 1300
        win_height = 900
        ...
        # Layout Grid: Spalte 1 (Rechts) soll wachsen
        self.grid_columnconfigure(1, weight=1)
        
        # Zeile 0: Toolbar, Zeile 1: Queue (groß), Zeile 2: Platzhalter, Zeile 3: Log
        self.grid_rowconfigure(0, weight=0)  # Toolbar
        self.grid_rowconfigure(1, weight=10) # Queue
        self.grid_rowconfigure(2, weight=0)  # Platzhalter
        ...
        self.logo_label = ctk.CTkLabel(self.sidebar, text="Steuerung", font=ctk.CTkFont(size=20, weight="bold"))
        self.logo_label.pack(padx=20, pady=(20, 10))

        # Queue Starten + Hinzufügen direkt unter "Steuerung"
        self.start_btn = ctk.CTkButton(self.sidebar, text="▶ Queue Starten", fg_color="green", height=40, font=ctk.CTkFont(size=14, weight="bold"), command=self.start_queue)
        self.start_btn.pack(padx=20, pady=(0, 25), fill="x")

        self.add_btn = ctk.CTkButton(self.sidebar, text="➕ Hinzufügen", command=self.add_manual_job)
        self.add_btn.pack(padx=20, pady=(0, 10), fill="x")

        ctk.CTkLabel(self.sidebar, text="Manueller Eintrag:", text_color="gray").pack(anchor="w", padx=20, pady=(10, 0))

        self.url_entry = ctk.CTkEntry(self.sidebar, placeholder_text="URL einfügen...")
        self.url_entry.pack(padx=20, pady=5, fill="x")

        # Zeitabschnitt-Panel mit individuellen HH:MM:SS Feldern
        self.time_panel = TimeInputPanel(self.sidebar)
        self.time_panel.pack(padx=20, pady=5, fill="x")

        # Modus Auswahl (Video/Audio)
        ...
        # Qualität & Format Dropdowns
        ctk.CTkLabel(self.sidebar, text="Qualität/Auflösung:", text_color="gray", font=("Arial", 11)).pack(anchor="w", padx=20, pady=(10, 0))
        ...
        ctk.CTkLabel(self.sidebar, text="Seitenverhältnis:", text_color="gray", font=("Arial", 11)).pack(anchor="w", padx=20, pady=(5, 0))
        ...
        self.output_dir_label = ctk.CTkLabel(
            ...
            text=self._short_path(Config.CUSTOM_OUTPUT_DIR) if Config.CUSTOM_OUTPUT_DIR else "(Standard)",
        ...
        self.cookie_combo = ctk.CTkComboBox(
            ...
            values=["Keine", "Browser: Chrome", "Browser: Firefox", "Browser: Edge", "Browser: Brave", "Cookie-Datei (Settings)"]
        )
        self.cookie_combo.pack(side="left")

        # Import urls.txt (mitte-links)
        ...
        # Fehler Button (rechts)
        self.error_btn = ctk.CTkButton(self.toolbar, text="⚠️ Fehler (0)", fg_color="#333333", hover_color="#444444", command=self.show_error_window)
        self.error_btn.pack(side="right", padx=(10, 0))

        # yt-dlp Update (rechts, vor Fehler)
        ...
        # --- 3. JOB LISTE (Rechts Mitte - Groß) ---
        self.scroll_frame = ctk.CTkScrollableFrame(self, label_text="Warteschlange (Queue)")
        self.scroll_frame.grid(row=1, column=1, padx=20, pady=(5, 5), sticky="nsew")
        
        # --- 4. LOG OUTPUT (Rechts Unten - Kleiner) ---
        ...
        self.log_textbox.insert("0.0", f"--- System Ready | Speicherort: {self.get_output_dir()} ---\n")

        self.is_running = False
        self.jobs = []
        self.failed_jobs = [] # Liste für fehlgeschlagene Jobs (URL, Mode, Time, Quality, Format)
    ...
    def _cookie_setting_to_display(self):
        """Wandelt Config Cookie-Einstellungen in Dropdown-Text um."""
        mode = Config.COOKIE_MODE
        if mode == "browser":
            browser = Config.COOKIE_BROWSER.capitalize()
            return f"Browser: {browser}"
        elif mode == "file":
            return "Cookie-Datei (Settings)"
        return "Keine"

    def browse_output_dir(self):
        """Öffnet einen Ordner-Dialog zur Auswahl des Zielordners."""
        ...
        if folder:
            ...
            self.log(f"Speicherort: {folder}", "SYS")
    ...
    def get_output_dir(self):
        """Gibt den aktuellen Speicherort zurück (eigener oder Standard)."""
    ...
    def ensure_output_dirs(self):
        """Stellt sicher dass der gewählte Zielordner existiert."""
        output = self.get_output_dir()
        if not os.path.exists(output):
            os.makedirs(output, exist_ok=True)

    def update_output_display(self):
        """Loggt den neuen Speicherort wenn Checkbox geändert wird."""
        self.log(f"Speicherort: {self.get_output_dir()}", "SYS")
    ...
    def _build_cookie_args(self, cookie_selection):
        ...
        if cookie_selection.startswith("Browser:"):
            ...
            if cookie_path and os.path.exists(cookie_path):
                ...
                self.log(f"Warnung: Cookie-Datei nicht gefunden: {cookie_path}", "WARN")
        return args
    ...
    def _fetch_job_metadata(self, job):
        try:
            ...
            self.log(f"Metadaten nicht geladen für {job.url}: {e}", "META")
    ...
    def request_skip(self, job_widget):
        if not self.is_running or self.current_job is not job_widget:
            self.log("Skip nur für den aktuell laufenden Job möglich.", "WARN")
            return
        self.skip_requested = True
        job_widget.set_detail("Skip angefordert...")
        self.log(f"Skip angefordert: {job_widget.url}", "SKIP")
    ...
    def _refresh_error_button(self):
        ...
        self.error_btn.configure(text=f"⚠️ Fehler ({err_count})")
    ...
    def _prepare_job_for_download(self, job, cookie_selection, output_dir, include_thumbnail=True):
        ...
        if self._output_already_exists(output_dir, display_name):
            ...
            job.set_detail("Bereits im Zielordner vorhanden")
            job.set_progress(1.0)
            self.log(f"Skip vorhanden: {display_name}", "SKIP")
    ...
    def delete_job(self, job_widget):
        if self.is_running:
            self.log("Löschen gesperrt während Queue läuft!", "WARN")
            return
        if job_widget in self.jobs:
            self.jobs.remove(job_widget)
            job_widget.destroy()
            self.log("Eintrag entfernt.", "DEL")
    ...
    def add_manual_job(self):
        ...
        if not url: return
        # NEU: Sections aus dem TimeInputPanel holen
    ...
    def import_from_file(self):
        if not os.path.exists(Config.URLS_FILE):
            self.log(f"Datei fehlt: {Config.URLS_FILE}", "ERR")
            return
        if self.is_running:
            self.log("Import gesperrt während Queue läuft.", "WARN")
            return
        ...
        try:
            ...
            for line in lines:
                ...
                count += 1
            self.log(f"{count} Jobs importiert.", "IMPORT")
        except Exception as e:
            self.log(f"Fehler Import: {e}", "ERR")

    def update_ytdlp(self):
        self.update_btn.configure(state="disabled", text="Lade Update...")
        self.log("Starte yt-dlp Update...", "SYS")
        ...
        def run_update():
            try:
                ...
                self.log("Update Vorgang beendet.", "SYS")
            except Exception as e:
                self.log(f"Update Fehler: {e}", "ERR")
            finally:
    ...
    def start_queue(self):
        ...
        if not pending:
            self.log("Keine offenen Jobs.", "SYS")
            return
    ...
    def process_queue(self, cookie_setting):
        ...
        for i, job in enumerate(pending_jobs):
            ...
            # 2. PAUSE (Wenn nicht der letzte Job)
            if i < len(pending_jobs) - 1 and result != "skipped":
                wait_time = random.randint(5, 15)
                self.log(f"⏳ Warte {wait_time} Sekunden (Schutz vor Ban)...", "WAIT")
                
                # Schrittweise warten, damit "Abbruch" möglich bleibt
                for _ in range(wait_time):
                    if not self.is_running: break
                    time.sleep(1)
        
        self.is_running = False
        self.start_btn.configure(state="normal", text="▶ Queue Starten")
        self.import_btn.configure(state="normal")
        self.log("Alle Aufgaben erledigt.", "FINISH")

    def _build_base_cmd(self, job, output_template, cookie_selection):
        """Erstellt den Basis-Befehl für yt-dlp (ohne URL und ohne Section)."""
        cmd = [Config.YT_DLP_PATH]
        ...
        if job.mode == "mp3":
            ...
            ext = job.fmt.lower()
            # H264-Codec bevorzugen (avc1), Fallback auf beliebigen Codec im gewünschten Container
            if ext == "mp4":
                h264_pref = "vcodec~='^(avc|h264)'"  # H264 bevorzugen
            else:
                h264_pref = ""  # Für MKV/WEBM kein Codec-Filter
    ...
    def _handle_ytdlp_output_line(self, job, line):
        ...
        if lower.startswith('[download] destination:'):
            ...
            job.set_detail("DL | Merging Formate...")
        elif '[extractaudio]' in lower:
            job.set_detail("DL | Konvertiere Audio...")
        elif '[ffmpeg]' in lower:
            job.set_detail("DL | FFmpeg Verarbeitung...")
    ...
    def _run_ytdlp(self, job, cmd):
        """Führt einen yt-dlp Befehl aus, loggt live und aktualisiert den Fortschritt."""
    ...
    def _find_downloaded_file(self, output_dir, pattern_prefix):
        """Findet die heruntergeladene Datei anhand des Präfix-Musters."""
    ...
    def _merge_with_ffmpeg(self, part_files, output_file, output_dir):
        """Fügt mehrere Videodateien mit FFmpeg concat zusammen (ohne Qualitätsverlust)."""
        concat_path = os.path.join(output_dir, f"_concat_{int(time.time())}.txt")
        try:
            # Concat-Liste erstellen
            with open(concat_path, 'w', encoding='utf-8') as f:
                for pf in part_files:
                    # FFmpeg erwartet forward slashes oder escaped backslashes
                    safe_path = pf.replace('\\', '/')
                    f.write(f"file '{safe_path}'\n")

            self.log(f"🔗 Merge: {len(part_files)} Teile → {os.path.basename(output_file)}", "MERGE")
            ...
            if result.returncode == 0:
                self.log(f"✅ Merge erfolgreich: {os.path.basename(output_file)}", "MERGE")
                # Aufräumen: Einzelteile löschen
                for pf in part_files:
                    try: os.remove(pf)
                    except: pass
                self.log(f"🗑️ {len(part_files)} Einzelteile gelöscht.", "MERGE")
                return True
            else:
                self.log(f"❌ Merge fehlgeschlagen: {result.stdout[-500:]}", "ERR")
                return False
        except Exception as e:
            self.log(f"❌ Merge Fehler: {e}", "ERR")
            return False
        finally:
            # Concat-Datei immer aufräumen
    ...
    def run_single_job(self, job, cookie_selection):
        ...
        # Prüfen ob mehrere Sections vorhanden sind
        sections = []
        if job.section:
        ...
        try:
            if len(sections) > 1:
                ...
                self.log(f"📋 {len(sections)} Zeitabschnitte erkannt – starte Multi-Download...", "DL")
                ...
                for idx, sec in enumerate(sections):
                    ...
                    self.log(f"⬇️ Teil {idx+1}/{len(sections)}: {sec}", "DL")
                    returncode = self._run_ytdlp(job, cmd)

                    if returncode == "skipped":
                        job.set_status("skipped")
                        job.set_detail("DL | Übersprungen")
                        self._refresh_error_button()
                        job.set_skip_enabled(False)
                        self.current_job = None
                        self.current_process = None
                    ...
                    if returncode != 0:
                        self.log(f"❌ Teil {idx+1} fehlgeschlagen!", "ERR")
                        all_ok = False
                        break
                    ...
                    if found:
                        ...
                        self.log(f"✅ Teil {idx+1}: {os.path.basename(found)}", "DL")
                    else:
                        self.log(f"⚠️ Teil {idx+1}: Datei nicht gefunden!", "WARN")
                        all_ok = False
                        break

                if all_ok and len(part_files) > 1:
                    # Finale Datei: Name des ersten Teils ohne Part-Suffix
                ...
                # ====== SINGLE SECTION oder KEIN SECTION (wie bisher) ======
...
class TimeSegmentWidget(ctk.CTkFrame):
    """Ein einzelner Zeitabschnitt mit Start- und End-Zeit (HH:MM:SS Felder)."""
    def __init__(self, master, remove_callback=None, show_remove=True):
        ...
        self.all_entries = []  # Alle 6 Felder flach: [start_h, start_m, start_s, end_h, end_m, end_s]
        self._vars = []       # StringVars für alle 6 Felder
        ...
        # Entfernen-Button oder Dash-Label
    ...
    def _create_time_row(self, parent):
        """Erstellt eine HH:MM:SS Eingabezeile und fügt die Felder zu all_entries hinzu."""
        entries = []
        for i in range(3):
            var = ctk.StringVar()
            ...
            # Globalen Index berechnen (0-5 über beide Zeilen)
            global_idx = len(self.all_entries)
            self.all_entries.append(entry)
            self._vars.append(var)

            # Auto-Tab: nach 2 Zeichen zum nächsten Feld springen (über beide Zeilen hinweg)
    ...
    def _validate_key(self, event):
        """Erlaubt nur Ziffern, Tab, Backspace, Delete, Pfeiltasten."""
    ...
    def _on_input(self, var, global_idx):
        """Auto-Tab nach 2 Zeichen — springt auch von Start-SS zu End-HH."""
        val = var.get()
        ...
        if len(digits) > 2:
            var.set(digits[:2])
        if len(digits) >= 2:
            # Zum nächsten Feld springen (0→1→2→3→4→5)
            if global_idx < len(self.all_entries) - 1:
                self.all_entries[global_idx + 1].focus_set()

    def get_section(self):
        """Gibt den Zeitabschnitt als 'HH:MM:SS-HH:MM:SS' String zurück, oder None wenn leer."""
...
class TimeInputPanel(ctk.CTkFrame):
    """Panel das mehrere TimeSegmentWidgets verwaltet mit einem + Button."""
    def __init__(self, master):
        super().__init__(master, fg_color="transparent")
        self.segments = []

        # Container für die Segmente
        self.segments_frame = ctk.CTkFrame(self, fg_color="transparent")
        self.segments_frame.pack(fill="x")

        # Label
        ctk.CTkLabel(self, text="Zeitabschnitte (optional):", text_color="gray",
                     font=("Arial", 11)).pack(anchor="w", pady=(0, 3))

        # Erstes Segment (ohne Entfernen-Button)
        self._add_segment(show_remove=False)

        # Plus-Button
        self.add_btn = ctk.CTkButton(self, text="➕ Weiterer Abschnitt", height=24,
                                      fg_color="#444444", hover_color="#555555",
                                      font=("Arial", 11), command=self._add_with_remove)
        self.add_btn.pack(fill="x", pady=(3, 0))

    def _add_segment(self, show_remove=True):
        """Fügt ein neues TimeSegmentWidget hinzu."""
    ...
    def _add_with_remove(self):
        """Fügt ein entfernbares Segment hinzu."""
    ...
    def get_sections(self):
        """Gibt eine Liste aller gültigen Zeitabschnitte zurück."""
        result = []
    ...
    def clear(self):
        """Entfernt alle Segmente und erstellt ein neues leeres."""
        for seg in self.segments:
            seg.destroy()
        self.segments.clear()
        self._add_segment(show_remove=False)
...
class JobWidget(ctk.CTkFrame):
    def __init__(self, master, id, url, mode, section, quality, fmt, ratio, encoder, remove_callback, skip_callback):
        ...
        self.thumb_label = ctk.CTkLabel(self, text="Kein\nBild", width=96, height=54, corner_radius=6, fg_color="#1b1b1b")
        self.thumb_label.grid(row=0, column=2, padx=(8, 10), pady=6)

        self.lbl_status = ctk.CTkLabel(self, text="Wartend", text_color="gray", width=80)
    ...
    def set_status(self, status):
        ...
        if status == "running":
            self.lbl_status.configure(text="⏳ Lädt...", text_color="#E0A800")
        elif status == "done":
            self.lbl_status.configure(text="✅ Fertig", text_color="#2CC985")
            self.progress_bar.set(1.0)
        elif status == "error":
            self.lbl_status.configure(text="❌ Fehler", text_color="#FF4D4D")
        elif status == "skipped":
            self.lbl_status.configure(text="⏭ Überspr.", text_color="#D0A13A")

class ErrorWindow(ctk.CTkToplevel):
    def __init__(self, master, failed_jobs, retry_callback):
        super().__init__(master)
        self.title("Fehlerhafte Downloads")
        self.geometry("600x400")
        self.retry_callback = retry_callback
        self.failed_jobs = failed_jobs

        ctk.CTkLabel(self, text=f"{len(failed_jobs)} Downloads fehlgeschlagen", font=("Arial", 16, "bold")).pack(pady=10)
        ...
        ctk.CTkButton(btn_frame, text="📋 Kopieren", command=self.copy_to_clipboard).pack(side="left", padx=10)
        ctk.CTkButton(btn_frame, text="🔄 Erneut versuchen", fg_color="green", command=self.retry_all).pack(side="right", padx=10)
...
class SettingsWindow(ctk.CTkToplevel):
    """Einstellungsfenster für alle konfigurierbaren Pfade und Optionen."""
    def __init__(self, master):
        super().__init__(master)
        ...
        ctk.CTkLabel(main, text="⚙ Einstellungen", font=ctk.CTkFont(size=18, weight="bold")).pack(pady=(0, 15))

        # --- Pfade ---
        ctk.CTkLabel(main, text="Pfade", font=ctk.CTkFont(size=14, weight="bold"), text_color="#6EA4FF").pack(anchor="w", pady=(10, 5))
        ...
        self._add_path_row(main, "yt-dlp Pfad:", self.ytdlp_var, filetypes=[("Executable", "*.exe"), ("Alle", "*.*")])

        # urls.txt Pfad
        self.urls_var = ctk.StringVar(value=Config.URLS_FILE)
        self._add_path_row(main, "URLs-Datei:", self.urls_var, filetypes=[("Text", "*.txt"), ("Alle", "*.*")])

        # --- Cookies ---
        ctk.CTkLabel(main, text="Cookies / Authentifizierung", font=ctk.CTkFont(size=14, weight="bold"), text_color="#6EA4FF").pack(anchor="w", pady=(20, 5))

        # Cookie Modus
        cookie_mode_frame = ctk.CTkFrame(main, fg_color="transparent")
        cookie_mode_frame.pack(fill="x", pady=3)
        ctk.CTkLabel(cookie_mode_frame, text="Modus:", width=150, anchor="w").pack(side="left")
        ...
        ctk.CTkLabel(profile_frame, text="Browser-Profil (optional):", width=150, anchor="w").pack(side="left")
        self.cookie_profile_var = ctk.StringVar(value=Config.COOKIE_BROWSER_PROFILE)
        ctk.CTkEntry(profile_frame, textvariable=self.cookie_profile_var, placeholder_text="z.B. Profile1 oder leer lassen").pack(side="left", fill="x", expand=True)

        # Cookie Datei
        self.cookie_file_var = ctk.StringVar(value=Config.COOKIE_FILE_PATH)
        self._add_path_row(main, "Cookie-Datei:", self.cookie_file_var, filetypes=[("Text", "*.txt"), ("Alle", "*.*")])
        ...
        ctk.CTkButton(
            btn_frame, text="💾 Speichern", fg_color="green", hover_color="#228B22",
            font=ctk.CTkFont(size=13, weight="bold"), height=36,
            command=self.save_and_close
        ).pack(side="right", padx=5)

        ctk.CTkButton(
            btn_frame, text="Abbrechen", fg_color="#555555", hover_color="#444444",
            height=36, command=self.destroy
        ).pack(side="right", padx=5)

        ctk.CTkButton(
            btn_frame, text="🔄 Zurücksetzen", fg_color="#8B4513", hover_color="#A0522D",
    ...
    def _add_path_row(self, parent, label, var, is_dir=False, filetypes=None):
        ...
        def browse():
            if is_dir:
                ...
                path = filedialog.askopenfilename(title=label, filetypes=filetypes or [("Alle", "*.*")])
            if path:
                var.set(path)
    ...
    def save_and_close(self):
        ...
        if hasattr(self.master_app, 'output_dir_label'):
            display = self.master_app._short_path(Config.CUSTOM_OUTPUT_DIR) if Config.CUSTOM_OUTPUT_DIR else "(Standard)"
        ...
        self.master_app.log("Einstellungen gespeichert.", "SYS")
        self.destroy()
